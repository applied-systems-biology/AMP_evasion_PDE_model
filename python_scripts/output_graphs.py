...
if "proba_survival.csv" in os.listdir(path):
    df = pd.read_csv(path + "/proba_survival.csv", sep = ";")
    fig = px.line(df, x="time", y="proba_survival", line_shape="spline", markers=True)
    fig.update_layout(yaxis_title = "Proba_survival", xaxis_title = "time")
    with open(path+"/proba_survival.html", "a+") as f:
        f.write(fig.to_html(full_html=False, include_plotlyjs='cdn', include_mathjax='cdn'))