...
ConfigParameters getMainConfigParameters(const std::string &main_config) {
  ...
  parameters.runs = json_parameters["AMP_evasion"].value("runs", 1);
  parameters.config_path = json_parameters["AMP_evasion"].value("config_path", "");
  parameters.output_dir = json_parameters["AMP_evasion"].value("output_dir", "/tmp/results");
  parameters.conda_dir = json_parameters["AMP_evasion"].value("conda_dir", "");
  parameters.number_of_threads = json_parameters["AMP_evasion"].value("number_of_threads", 1);
  std::mt19937 gen(std::random_device{}());
  std::uniform_int_distribution dis;
  parameters.system_seed = json_parameters["AMP_evasion"].value("seed", dis(gen));
  for (const auto &item: json_parameters["AMP_evasion"]["parameter_screening"].items()) {
  ...
  parameters.screen_start_idx = json_parameters["AMP_evasion"].value("screen_start_idx", 1);
  json_file.close();
  return parameters;
}
...
OutputParameters getOutputParameters(const std::string &output_config) {
  ...
  json json_parameters;
  json_file >> json_parameters;
  parameters.activated = json_parameters["AMP_evasion"].value("activated", false);
  parameters.output_graphs = json_parameters["AMP_evasion"].value("output_graphs", false);
  parameters.output_ovito = json_parameters["AMP_evasion"].value("output_ovito", false);
  json_file.close();
  return parameters;
}
SimulationParameters getSimulationParameters(const std::string &simulator_config) {
  ...
  json_file >> json_parameters;
  try {
    parameters.topic = json_parameters["AMP_evasion"]["topic"];
    parameters.dimensions = json_parameters["AMP_evasion"]["dimensions"];
    parameters.max_time = json_parameters["AMP_evasion"]["max_time"];
    parameters.time_stepping = json_parameters["AMP_evasion"]["timestepping"];
    parameters.dt_auto = json_parameters["AMP_evasion"].value("dt_auto", false);
  } catch (const std::bad_optional_access &e) {
    ERROR_STDERR(e.what());
    throw;
  }
  for (const auto &site: json_parameters["AMP_evasion"]["Sites"]) {
    ...
    for (const auto& agent_type:site["AgentManager"]["Types"]) {
      ...
      agent_parameters->death = agent.value("death", false);
...
AnalyserParameters getAnalyserParameters(const std::string &analyser_config) {
  ...
  json json_parameters;
  json_file >> json_parameters;
  parameters.active_measurements = json_parameters["AMP_evasion"]["active_measurements"].get<std::unordered_set<std::string>>();
  parameters.cell_state_count = json_parameters["AMP_evasion"]["cell_state_count"].get<std::vector<std::string>>();
  parameters.output_every_X_time_unit = json_parameters["AMP_evasion"].value("output_every_X_time_unit", 0.0);
  json_file.close();
  return parameters;
}
...
InputParameters getInputParameters(const std::string &input_config) {
  ...
  for (auto &rate:json_parameters["AMP_evasion"]["Rates"].items()) {
