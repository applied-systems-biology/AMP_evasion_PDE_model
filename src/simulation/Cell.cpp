...
void Cell::interactWithMolecules(double timestep, double current_time) {
    ...
    if (mol_layer){
        for (const auto sphere : agentProps->getMorphology()->getAllSpheresOfThis()) {
            ...
            for (auto& [mol_name, tmp] : secretion_rate){
                if (current_time >= secretion_delay[mol_name]) {
                    secretion_start = true;
                }
            }
...
void Cell::setup(double time_delta, double current_time, abm::util::SimulationParameters::AgentParameters *parameters) {
    ...
    //TODO: read from csv/text file (or json?)
    fitted_proba_survival_relation_[0.01] = 3.2839e-4;
    fitted_proba_survival_relation_[0.1] = 8.7374e-5;
    fitted_proba_survival_relation_[1.0] = 8.2531e-5;
    fitted_proba_survival_relation_[10.0] = 8.2531e-5;
    fitted_proba_survival_relation_[100.0] = 8.2531e-5;

    death_ = parameters->death;
    proba_survival_ = 1.0;
}
...
void Cell::update_survival_probability(){
    auto k = fitted_proba_survival_relation_[uptake_rate["AMP"]];
    auto volume = (site->getUpperLimits().x - site->getLowerLimits().x) * (site->getUpperLimits().y - site->getLowerLimits().y) * (site->getUpperLimits().z - site->getLowerLimits().z);
    auto inside_conc_val = get_inside_conc()["AMP"]/volume;
    proba_survival_ = exp(-k * inside_conc_val);
}

bool Cell::check_for_death(double current_time, double dt) {
    auto dead = false;
    auto p = site->getRandomGenerator()->generateDouble();
    auto proba_death = (1-proba_survival_)*dt;
    if (p < proba_death){
        dead = true;
        SYSTEM_STDOUT("The pathogen cell has died. It could not escaped the AMP. Victory for the host!");
    }
    return dead;
}